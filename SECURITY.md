...
- Enforce `https://` only for external checkout links (DB constraint + runtime URL normalization).

## Data access hardening

- Avoid broad client-side table reads when aggregate data is sufficient.
- Use RPC/view-based aggregate reads for public counters (for example, poll totals) to reduce data exposure and load.
